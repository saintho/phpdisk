...
<tr>
	...
	<?php if($auth[pd_a] && $settings[mod_subdomain]){ ?>
...
<table align="center" width="100%" cellpadding="4" cellspacing="0" border="0" class="td_line">
	...
	<tr>
		<td>提现密码：</td>
		<td><input type="password" name="income_pwd" value="" /> <?php if($same_pwd){ ?><br /><span class="txtred">安全提示：您的提现密码与登录密码相同，为保障您的资金安全，请及时<a href="<?=urr("mydisk","item=profile&action=mod_pwd")?>">修改提现密码</a>.</span><?php } ?></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
...
<?php if(!get_profile($pd_uid,'credit') && !get_profile($pd_uid,'dl_credit')){ ?>
getId('chg_btn').disabled = true;
<?php } ?>
</script>
<?php }elseif($action=='income_log'){ ?>
...
<?php }elseif($action=='income_plans'){ ?>
...
<tr>
	...
	<?php if($auth[open_plan_active] && $settings[open_plan_active]){ ?>
...
<?php 
 ...
 ?>
<tr
<?php if(!($auth[open_plan_active] && $settings[open_plan_active])){ ?>onclick="hl_plan(<?=$k?>,<?=count($plans)?>);"<?php } ?> style="cursor:pointer" id="tr_<?=$k?>"
<?php if($v[plan_id]==get_profile($pd_uid,'plan_id')){ ?>class="color4 txtblue"<?php } ?>>
	<td>
	<?php if(get_profile($pd_uid,'open_plan') && !$settings[open_plan_active]){ ?>
...
<tr>
	<td>&nbsp;</td>
	<td>
	<?php if(get_profile($pd_uid,'open_plan')){ ?>
...
<?php if($task =='forum_upload_super'){ ?>
<tr>
	<td><?=__('add_code_is')?></td>
...
<?php }elseif($action=='dl_users' || $action=='dl_users2'){ ?>
<table align="center" width="100%" cellpadding="4" cellspacing="0" border="0" class="td_line">
<tr>
...
<?php }elseif($action=='guest' && menu_guest_reg()){ ?>
<div id="container">
<div class="box_style">
<?php if($can_edit){ ?>
