...
// Cache Time:2015-10-08 15:33:44
...
<div class="admin_header">
	...
	<div id="sitemap">
	<?=__('current_admin')?>: <span class="txtred"><?=$pd_username?></span>[<?php if(super_admin()){ ?><?=__('super_admin')?><?php }else{ ?><?=__('common_admin')?><?php } ?>]&nbsp;
...
<script type="text/javascript">
<?php if(in_array($item,array('main','settings'))){ ?>
$('#my_settings').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('users','plans','vip','domain','email','verycode'))){ ?>
$('#my_users').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('files','public','report'))){ ?>
$('#my_files').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('plugins'))){ ?>
$('#my_plugins').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('templates','lang'))){ ?>
$('#my_templates').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('advertisement','link','announce','navigation','credit','comment','seo'))){ ?>
$('#my_advertisement').addClass('admin_nav_sel');
<?php }elseif(in_array($item,array('database','cache'))){ ?>
...
<table height="100%" cellspacing="0" cellpadding="0" width="100%" border="0" style="background:#FCFCFC;">
  <tr>
    <td valign="top" width="150" id="admincp_left_frame" >
	  <?php require_once template_echo('menu',$admin_tpl_dir); ?>
	  </td>
	  ...
	  <div style="padding:200px 0;"><img id="menu_img" align="absmiddle" src="images/menu_left.gif" border="0"></div>
		...
		<td valign="top" width="100%" id="admincp_main_frame">
			<?php require_once $action_module; ?>
		</td>
		</tr>
	</table>
</div>	
