...
<!--#if($pd_gid==1){#-->
<!--#if($curr_script=='index'){#-->
<div class="adm_cp" id="adm_cp">
<fieldset><legend><?=__('diy_index_tips')?></legend>
<form id="frm_index">
<input type="hidden" name="action" value="cp_index" />
<!--#if($auth[close_guest_upload] && !$settings['close_guest_upload']){#-->
...
<input type="button" onclick="getId('adm_cp').style.display='none';" title="<?=__('close')?>" value=" X " />
