...
#
#	$Id: phpmailer.class.php 121 2014-03-04 12:38:05Z along $
#
...
class phpmailer {
	...
	function SmtpSend($header, $body) {
		...
		if(count($bad_rcpt) > 0) {       for($i = 0; $i < count($bad_rcpt); $i++) {
			if($i != 0) {
				$error .= ', ';
			}
			$error .= $bad_rcpt[$i];
	...
	function CreateHeader() {
		...
		$result .= $this->HeaderLine('From', $this->FromName.' <'.$this->FromEmail.'>');
	...
	function CreateBody() {
		...
		switch($this->message_type) {
			...
			case 'alt_attachments':
				...
				$result .= $this->GetBoundary($this->boundary[2], '', 'text/plain', '') . $this->LE;         $result .= $this->EncodeString($this->AltBody, $this->Encoding);
				$result .= $this->LE.$this->LE;
				$result .= $this->GetBoundary($this->boundary[2], '', 'text/html', '') . $this->LE;         $result .= $this->EncodeString($this->Body, $this->Encoding);
				$result .= $this->LE.$this->LE;
